...
if (process.env.GMAIL_USER && process.env.GMAIL_APP_PASSWORD) {
  const smtpOptions = process.env.SMTP_HOST
    ...
    : { service: 'gmail' };
  mailer = nodemailer.createTransport({ ...smtpOptions, auth: { user: process.env.GMAIL_USER, pass: process.env.GMAIL_APP_PASSWORD }, connectionTimeout: 10000, greetingTimeout: 10000, socketTimeout: 15000 });
}

async function sendNotificationEmail({ userId, role, title, body, imageUrl }) {
  if (!mailer || !['student', 'driver'].includes(role)) return;
  ...
  await mailer.sendMail({ from: `"Zira Go" <${process.env.GMAIL_USER}>`, to: email, subject: title, text: body, html: `<div style="font-family:Arial,sans-serif;max-width:540px;margin:auto;padding:24px;color:#0f172a">${image}<h2 style="color:#6d28d9">${title}</h2><p style="white-space:pre-line;line-height:1.55">${body}</p><p style="color:#64748b;font-size:12px">Zira Go · Landmark University</p></div>` });
}
